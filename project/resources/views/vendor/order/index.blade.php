...
@endphp
                                      ...
                                      <td>
                                        ...
                                        <a href="{{route('vendor-order-show',$order->order->order_number)}}" class="btn btn-primary product-btn"><i class="fa fa-eye"></i> {{ $langg->lang539 }}</a>
                                            <select class="vendor-btn nice {{ $order->status == 'ready to ship' ? 'completed' : $order->status }}">
                                            ...
                                            <option value="{{ route('vendor-order-status',['slug' => $order->order->order_number, 'status' => 'ready to ship']) }}" {{  $order->status == "ready to ship" ? 'selected' : ''  }}>Ready To ship</option>
                                        ...
                                        <td>
                                           @php
                                               $a = \App\Models\Order::where('order_number', $order->order->order_number)->first();
                                           @endphp
                                           @if ($a->pdf_one != null)
                                           <a href="{{ url('/assets/pdf/'.$a->pdf_one) }}">
                                            Download invoice
                                        </a>
                                        @else
                                        ---
                                           ...
                                           @if ($a->pdf_two != null)
                                           <a href="{{ url('/assets/pdf/'.$a->pdf_two) }}">
                                            Download update
                                        </a>
                                        @else
                                        ---
