...
class OrderController extends Controller
{
    ...
    public function datatables($status)
    {
        ...
        elseif($status == 'declined') {
         ...
         return Datatables::of($datas)
                            ...
                            ->addColumn('Update', function(Order $data){
                                return '<div class="godropdown"><a href="'.route('admin-order-update-custom', $data->id).'" class="go-dropdown-toggle">Update Order</a></div>';
                            })
                            ->addColumn('After', function(Order $data){
                                $a = $data->after_delivery == 'success' ? '-- -- --' : $data->after_delivery;
                                return$a;
                            })
                            ->addColumn('vendor_Status', function(Order $data){
                               return VendorOrder::VendorStatus($data->order_number);
                            return '-- -- -- ';
                            })
                            ->rawColumns(['id','action', 'Update', 'After', 'vendor_Status'])
    ...
    public function updateOrder($id){
        ...
        return view('custom.uploadfile', compact('data'));
    }



    public function updateOrderPost(Request $r, $id){

        function upload($file, $dest){
            $name = bin2hex(random_bytes(24));
            $ext = $file->getClientOriginalExtension();
            $full_name = $name.'.'.$ext;
            if($file->move($dest, $full_name)){
                return $full_name;
            }
            return false;
        }
       ...
       if($r->hasFile('data-one')){
        $pdf_one = upload($r->file('data-one'),'assets/pdf');
       }else{
        $pdf_one = $data->pdf_one;
       }

       if($r->hasFile('data-two')){
        $pdf_two = upload($r->file('data-two'),'assets/pdf');
       }else{
        $pdf_two = $data->pdf_two;
       }
        ...
        $a = Order::where('id', $id)->update([
            'pdf_one' => $pdf_one,
            'pdf_two' => $pdf_two
        ]);

        if($a){
            $msg = ['status' => 1, 'message' => 'pdf updated success'];
        }else{
            $msg = ['status' => 0, 'message' => 'pdf updated failed'];
        }
